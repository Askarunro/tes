...
export default class ApiService {
  ...
  async fetchMovie() {
    const response = await axios.get(
      `https://api.themoviedb.org/3/search/movie?api_key=${API_KEY}&query=${this.searchQuery}`,
    );
  ...
  async fetchMovieList() {
    const response = await axios.get(
      `https://api.themoviedb.org/3/genre/movie/list?api_key=${API_KEY}&query=${this.searchQuery}`,
    );
